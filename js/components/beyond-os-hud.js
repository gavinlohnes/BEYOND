 ...
 * Visual-only module. No state, no logic.
 */

const HUD_UI = {
  // Base Component: A standardized stat block
  ...
  // Base Component: Grid wrapper for stats
  ...
  // Complex Component: Exercise metrics card
    ...
    <div class="screen-stack animate-stagger">
      ...
      <nav class="nav-tabs animate-fade-up">
        ${['HUD', 'Today', 'Fuel', 'Meals', 'Prep'].map(t => `
          <button class="nav-tab ${data.activeView === t ? 'active' : ''}" data-view="${t}">
            ${t}
          </button>
        `).join('')}
      </nav>
